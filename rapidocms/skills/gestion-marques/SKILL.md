...
Un même compte peut opérer **plusieurs enseignes** (la liste vit dans
`./rapido-kb/entreprise.md` et se vérifie via `get_brand`). Dès qu'une action porte sur « la marque » (créer un contenu, un post, un
...
`"<Marque> — <type> — <variante>"` — ex. `Marque A — logo — fond transparent`,
`Marque A — logo — monochrome blanc`, `Marque B — visuel clé — bannière`. Cette
...
L'APPLICATION de la charte aux contenus (et la note pipeline vidéo — kit
« Mika ») reste dans `contenu-conforme-marque` : ce skill-ci définit
l'identité et son inventaire, l'autre l'impose aux productions.
