...
description: Utiliser quand l'utilisateur parle de « délivrabilité », dit « mes emails tombent en spam », « vérifie la liste avant envoi », « warmup », « on peut envoyer ? ». Gate OBLIGATOIRE avant tout lot d'envoi (scorecard de liste + contrôle spam de la copy) et runbook d'incident délivrabilité. Exécution via RapidoCRM.
---
...
`machine-outbound` **et** `rapidocrm:campagne-marketing` **doivent invoquer ce
skill avant tout lot d'envoi**. Un lot dont la liste est notée **sous le seuil
KB est REFUSÉ** — pas d'envoi tant que la liste n'est pas corrigée. (Règle posée
ici ; le câblage dans `rapidocrm:campagne-marketing` est prévu en M8-v2 ; il est
déjà écrit dans `machine-outbound`.)
...
### 3. Volumes & cadence (imposés)
- Respecter le **calendrier de montée en charge** et le **plafond quotidien** de
  `delivrabilite.md` : **jamais au-delà du plafond** ; fractionner un lot trop
  gros sur plusieurs jours.
